...
function checkPackageName (packageName = '') {
  ...
  if (!validForNewPackages) {
    ...
    allWarningsAndErros.forEach(e => console.log(chalk.red(` - ${e}`)))
    process.exit(1)
  }
}
...
function scriptLogs (projectName) {
  ...
  SCRIPTS.forEach(({ script, description }) => {
    console.log(chalk.bold.white(' *'), chalk.bold.cyan(script))
    console.log(' '.repeat(2), chalk.bold.white(description))
    console.log()
  })
}
